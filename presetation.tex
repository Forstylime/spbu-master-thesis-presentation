...
\author[Weiqian Xu]{Weiqian Xu \\ \vspace{0.2cm} \footnotesize Supervisor: Prof. Pankratova}
\institute[SPbU]{Saint Petersburg State University \\ \textit{Applied Mathematics and Control Processes Department}}
\date[2026]{2026}
...
%Currently, pixel-wise classification algorithms like EM or Random Forests are widely used. However, they share a critical statistical weakness: the i.i.d. assumption. They treat every single pixel as an independent sample. As highlighted in the red box, this directly violates Tobler's First Law of Geography. Natural landscapes are continuous and spatially autocorrelated. Because traditional models create a split between the physical feature space and geographic space, they inevitably produce spatial anomalies, which leads to the problem introduced on the next slide.
...
\begin{frame}{Data Overview: Spatial Distribution}
    ...
    \begin{itemize}
        \item The continuous physical field (a) shows attribute values range from 0.39 to 0.92, and shows a spatial gradient with clear spatial autocorrelation.
...
%             \item \alert{The Non-Spatial Problem:} Pixels with virtually identical physical values ($Z$) are forcefully assigned to different discrete classes by the EM classifier.
%         \end{itemize}

%         \begin{block}{Mechanism of Label Switching}
%             Marginal physical changes ($\Delta Z \approx 0.01$) due to natural variance or sensor noise trigger irregular categorical label switching (e.g., Class 3 $\to$ 4 $\to$ 3).
%         \end{block}
%     \end{columns}
% \end{frame}
...
\begin{frame}{Result 3: Expected Realities and Uncertainty}
    ...
    \begin{itemize}
        ...
        \item \textbf{Variance (b):} Uncertainty is spatially measured ($\sigma^2_K \in [0.36, 0.65]$). High variance zones correspond to areas with limited data support, guiding the subsequent hypothesis testing.
    \end{itemize}
\end{frame}
...
\begin{frame}{Result 4: Statistical Decisions}
    ...
    \begin{center}
        ...
        \begin{tabular}{lcc}
            ...
            \textbf{Rejected $H_0$ (Real Anomalies)}        & \textbf{6}           & \textbf{0.1\%}      \\
    ...
    \begin{itemize}
        ...
        \item Crucially, the conservative FDR test successfully identifies and \textbf{keeps 6 real micro-structures} that would have been removed by standard filters.
    \end{itemize}
\end{frame}
...
\begin{frame}{Result 5: Label Correction Analysis (MLE)}
    \begin{columns}[c]
        ...
        \begin{alertblock}{Key Insight}
            The Kriging-based MLE method effectively balances strong noise removal with the keeping of micro-scale geographic realities, resulting in a refined map that is both cleaner and more physically consistent.
        \end{alertblock}

        \column{0.48\textwidth}
        \centering
        \includegraphics[width=0.9\linewidth, keepaspectratio]{figures/thesis_single_c_diff.png}
        \captionof{figure}{\scriptsize Final Refined Categorical Difference Map.}
    \end{columns}
\end{frame}
...
\begin{frame}{Comprehensive Map Comparison}
    ...
    \begin{itemize}
        \item The Kriging-based refinement (b) successfully removes the "salt-and-pepper" noise while keeping original micro-features, resulting in a more physically consistent map.
        \item In contrast, the Majority Filter (c) reassigns 9,542 pixels and over-smooths the map, removing thin linear features and creating artificial block-like patterns.
    \end{itemize}
\end{frame}
